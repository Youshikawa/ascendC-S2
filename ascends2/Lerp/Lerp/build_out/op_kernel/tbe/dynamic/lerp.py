...
def _build_args(start_in__, end_in__, weight_in__, y_out_):
    __inputs__ = []
    for arg in [start_in__, end_in__, weight_in__]:
        if arg != None:
...
@tbe_register.register_operator("Lerp", trans_bool_to_s8=False)
...
def lerp(start_in__, end_in__, weight_in__, y_out_, kernel_name="lerp", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(start_in__, end_in__, weight_in__, y_out_)
    ...
    origin_func_name = "lerp"
    ascendc_src_dir_ex = "lerp"
    ascendc_src_dir = "lerp"
    ascendc_src_file = "lerp.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Lerp. kernel name is " + kernel_name)
    op_type = "Lerp"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[start_in__, end_in__, weight_in__], origin_outputs = [y_out_], param_type_dynamic = False,\
...
def op_select_format(start_in__, end_in__, weight_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(start_in__, end_in__, weight_in__, y_out_)
    result = check_op_cap("op_select_format", "Lerp", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(start_in__, end_in__, weight_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(start_in__, end_in__, weight_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "Lerp", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
