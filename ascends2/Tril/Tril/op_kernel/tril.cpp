...
template<typename TYPE_X1,typename TYPE_Y> class Tril {
    using T = TYPE_X1;
public:
    ...
    __aicore__ inline void Compute(int32_t progress, int32_t pos, uint32_t length) {
        ...
        //Select(y, ones, zeros, x1, SELMODE::VSEL_TENSOR_TENSOR_MODE, length);
         if (pos < 0) { // pos <= 0 == 切片首地址 -> 切片首地址大于等于pos 
            ...
            Select(y, this->ones, zeros, TYPE_X1(-1), SELMODE::VSEL_TENSOR_SCALAR_MODE, length);
         } else if (pos >= length - 1) { // pos >= length > length - 1 = 切片尾地址 全为0
            // Add(y, x1, zeros, length);
            Select(y, this->ones, x1, TYPE_X1(-2), SELMODE::VSEL_TENSOR_SCALAR_MODE, length);
         } else {
            // Add(y, zeros, zeros, length);
            // Add(y, x1, zeros, pos + 1);
            Select(y, this->ones, zeros, TYPE_X1(-3), SELMODE::VSEL_TENSOR_SCALAR_MODE, length);
            Select(y, this->ones, x1, TYPE_X1(-4), SELMODE::VSEL_TENSOR_SCALAR_MODE, pos + 1);
         }
        Q_x1.FreeTensor(x1); 
        Q_y.EnQue<TYPE_Y>(y);
    }
